...
## 🔑 المتغيرات المطلوبة (انسخ كل سطر)
...
### 2️⃣ AI Assistants ⚠️ **مهم جداً - المساعدين متوقفين!**
...
```
GOOGLE_AI_API_KEY
AIzaSyCjVqxv4vy8gXy3O0BNEp9dAB_UBKI2mh0
```
...
```
NEXT_PUBLIC_EOSDA_API_BASE_URL
https://api-connect.eos.com
```

```
NEXT_PUBLIC_EOSDA_API_VERSION
v1
```

```
NEXT_PUBLIC_EOSDA_CENTER_LAT
25.30084
```

```
NEXT_PUBLIC_EOSDA_CENTER_LNG
32.55524
```

```
NEXT_PUBLIC_EOSDA_DEFAULT_ZOOM
6
```

```
NEXT_PUBLIC_EOSDA_MIN_ZOOM
1
```

```
NEXT_PUBLIC_EOSDA_MAX_ZOOM
18
```

```
NEXT_PUBLIC_EOSDA_DEFAULT_CLOUD_COVERAGE
20
```
...
2. **لكل متغير**:
   - انقر على **"Add New"** أو **"Add Variable"**
   - انسخ **Key** (الاسم) من أعلاه
   - انسخ **Value** (القيمة) من أعلاه
   - اختر البيئات:
     - ✅ **Production**
     - ✅ **Preview**
     - ✅ **Development**
...
1. **المساعدين متوقفين**: تأكد من إضافة `GROQ_API_KEY` و `GOOGLE_AI_API_KEY` ⚠️
...
**إجمالي المتغيرات**: 32 متغير  
**المتغيرات المطلوبة**: 25 متغير  
**المتغيرات الاختيارية**: 7 متغيرات
