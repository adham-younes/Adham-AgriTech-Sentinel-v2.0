...
function resolveBounds(payload: NdviRequest): SentinelBounds {
...
export async function POST(request: Request) {
  try {
    ...
    const payload = (await request.json()) as NdviRequest
    const bounds = resolveBounds(payload)
    const date = payload.date ?? new Date().toISOString()

    const buffer = await fetchSentinelNdviImage({
      bounds,
      date,
    ...
    console.error("[Sentinel] NDVI endpoint failed", error)
    return NextResponse.json(
      { error: error?.message ?? "Failed to fetch Sentinel NDVI" },
      { status: 500 },
    )
  }
}
